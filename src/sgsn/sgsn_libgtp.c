					...
					struct pdp_t *pdp)
{
	...
	pctx = sgsn_pdp_ctx_alloc(mmctx, ggsn, pdp, pdp->nsapi);
	if (!pctx) {
...
static int cb_gtp_sgsn_context_request_ind(struct gsn_t *gsn, const struct sockaddr_in *peer, uint32_t local_ref, union gtpie_member * const *ie, unsigned int ie_size)
{
	...
	return gtp_sgsn_context_resp(gsn, local_ref, resp_ie);
}
...
static int cb_gtp_sgsn_context_response_ind(struct gsn_t *gsn, const struct sockaddr_in *peer, uint32_t local_ref, union gtpie_member * const *ie, unsigned int ie_size)
{
	struct sgsn_mm_ctx *mmctx = NULL;
	uint64_t imsi;
	uint8_t buf[512];
...
static int cb_gtp_sgsn_context_ack_ind(struct gsn_t *gsn, const struct sockaddr_in *peer, uint32_t local_ref, union gtpie_member * const *ie, unsigned int ie_size)
{
