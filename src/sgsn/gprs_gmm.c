...
static void vlr_subscr_inval_cb(void *ref, struct vlr_subscr *vsub, enum vlr_inval_reason reason)
{
	struct sgsn_mm_ctx *mmctx = ref;
	long data = 0;
	enum gsm48_reject_value cause;
	bool is_update_procedure = false;

	switch (reason) {
	case VLR_INVAL_REASON_UPDATE_LOCATION:	
		is_update_procedure = true;
		/* no cause for the MS */
		break;
	case VLR_INVAL_REASON_WITHDRAWN:
		is_update_procedure = false;
		data = cause = GSM48_REJECT_IMPLICITLY_DETACHED;
		break;
	case VLR_INVAL_REASON_LOCATION_EXPIRED:
		/* TODO: handle expired location correctly */
		is_update_procedure = false;
		data = cause = GSM48_REJECT_IMPLICITLY_DETACHED;
		break;
	case VLR_INVAL_REASON_DUPLICATE_SUBSCR:
...
static int vlr_tx_common_id_cb(void *ref)
{
	...
	if (ctx->ran_type == MM_CTX_T_UTRAN_Iu)
		return sgsn_ranap_iu_tx_common_id(ctx->iu.ue_ctx, ctx->imsi);
