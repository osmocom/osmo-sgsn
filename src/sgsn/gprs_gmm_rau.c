...
#include <osmocom/sgsn/gprs_gmm_rau.h>
...
static const struct osmo_tdef_state_timeout gmm_rau_fsm_timeouts[32] = {
	[ST_IDENTIY] = { .T=3370 },
	[ST_AUTH] = { .T=3360 },
	[ST_ACCEPT] = { .T=3350 },
	[ST_ASK_VLR] = { .T=3350 },
...
#define gmm_rau_fsm_state_chg(fi, NEXT_STATE) \
	osmo_tdef_fsm_inst_state_chg(fi, NEXT_STATE, gmm_rau_fsm_timeouts, sgsn->cfg.T_defs, -1)


static void st_init(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	struct sgsn_mm_ctx *ctx = fi->priv;
	struct msgb *rau_req = data;

	/* we can run st_init multiple times */
	if (ctx->gmm_rau_req.rau_req)
		msgb_free(ctx->gmm_rau_req.rau_req);

	ctx->gmm_rau_req.rau_req = msgb_copy(rau_req, "Attach Request");
	ctx->auth_state = SGSN_AUTH_UNKNOWN;
	ctx->gmm_rau_req.auth_reattempt = 0;
	 ...
	 */
	ctx->pending_req = GSM48_MT_GMM_RA_UPD_REQ;

	if (require_identity_imei) {
		ctx->gmm_rau_req.id_type = GSM_MI_TYPE_IMEI;
		gmm_rau_fsm_state_chg(fi, ST_IDENTIY);
	} else if (!strlen(ctx->imsi)) {
		ctx->gmm_rau_req.id_type = GSM_MI_TYPE_IMSI;
		gmm_rau_fsm_state_chg(fi, ST_IDENTIY);
	} else if (require_auth)
		gmm_rau_fsm_state_chg(fi, ST_AUTH);
	else
		gmm_rau_fsm_state_chg(fi, ST_ACCEPT);
}
...
static void st_identity_on_enter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	...
	switch (ctx->gmm_rau_req.id_type) {
	...
	ctx->t3370_id_type = ctx->gmm_rau_req.id_type;
	ret = gsm48_tx_gmm_id_req(ctx, ctx->gmm_rau_req.id_type);
	if (ret < 0) {
...
static void st_identity(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	if (type != ctx->gmm_rau_req.id_type) {
...
static void st_auth_on_enter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	...
	case SGSN_AUTH_UNKNOWN: /* the SGSN doesn know this MS */
		gmm_rau_fsm_state_chg(fi, ST_ASK_VLR);
		break;
	...
	case SGSN_AUTH_AUTHENTICATE:
		...
		ctx->gmm_rau_req.auth_reattempt++;
		break;
	}
}
...
static void st_auth(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case E_AUTH_RESP_RECV_SUCCESS:
		...
		if (ctx->ran_type == MM_CTX_T_UTRAN_Iu && !ctx->iu.ue_ctx->integrity_active)
			gmm_rau_fsm_state_chg(fi, ST_IU_SECURITY_CMD);
		else
#endif /* BUILD_IU */
			gmm_rau_fsm_state_chg(fi, ST_ACCEPT);
		break;
	case E_AUTH_RESP_RECV_RESYNC:
		if (ctx->gmm_rau_req.auth_reattempt <= 1)
			gmm_rau_fsm_state_chg(fi, ST_ASK_VLR);
		else
...
static void st_reject(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	if (reject_cause != GMM_DISCARD_MS_WITHOUT_REJECT)
		gsm48_tx_gmm_ra_upd_rej(ctx, (uint8_t) reject_cause);
...
static struct osmo_fsm_state gmm_rau_req_fsm_states[] = {
	/* default state for non-DTX and DTX when SPEECH is in progress */
	[ST_INIT] = {
		.in_event_mask = X(E_RAU_REQ_RECV),
	...
	[ST_ACCEPT] = {
		.in_event_mask = X(E_RAU_COMPLETE_RECV) | X(E_VLR_ANSWERED),
...
const struct value_string gmm_rau_req_fsm_event_names[] = {
	OSMO_VALUE_STRING(E_RAU_REQ_RECV),
	OSMO_VALUE_STRING(E_IDEN_RESP_RECV),
	OSMO_VALUE_STRING(E_AUTH_RESP_RECV_SUCCESS),
	OSMO_VALUE_STRING(E_AUTH_RESP_RECV_RESYNC),
	OSMO_VALUE_STRING(E_RAU_ACCEPTED),
	OSMO_VALUE_STRING(E_RAU_ACCEPT_SENT),
	OSMO_VALUE_STRING(E_RAU_COMPLETE_RECV),
...
void gmm_rau_allstate_action(struct osmo_fsm_inst *fi, uint32_t event, void *data) {
	struct sgsn_mm_ctx *ctx = fi->priv;
	struct msgb *new_rau_req = data;
	...
	case E_RAU_REQ_RECV:
		...
		case ST_RETRIEVE_AUTH:
			...
			/* FIXME: proper implement gmm rau req ies */
			if (ctx->gmm_rau_req.rau_req &&
					gprs_gmm_msg_cmp(new_rau_req, ctx->gmm_rau_req.rau_req)) {
				osmo_fsm_inst_state_chg(fi, ST_INIT, 0, 0);
				st_init(fi, event, data);
			}
			break;
...
int gmm_rau_timer_cb(struct osmo_fsm_inst *fi)
{
	struct sgsn_mm_ctx *ctx = fi->priv;
	struct gsm_auth_tuple *at = &ctx->auth_triplet;
	...
	case ST_IDENTIY:
		...
		gsm48_tx_gmm_id_req(ctx, ctx->gmm_rau_req.id_type);
...
struct osmo_fsm gmm_rau_req_fsm = {
	.name = "gmm_rau_REQ_FSM",
	.states = gmm_rau_req_fsm_states,
	.num_states = ARRAY_SIZE(gmm_rau_req_fsm_states),
	.event_names = gmm_rau_req_fsm_event_names,
	.allstate_event_mask = X(E_REJECT) | X(E_RAU_REQ_RECV),
	.allstate_action = gmm_rau_allstate_action,
	.log_subsys = DMM,
	.timer_cb = gmm_rau_timer_cb,
};

static __attribute__((constructor)) void gprs_gmm_fsm_init(void)
{
	OSMO_ASSERT(osmo_fsm_register(&gmm_rau_req_fsm) == 0);
}

void gmm_rau_req_free(struct sgsn_mm_ctx *mm) {
	if (mm->gmm_rau_req.fsm)
		osmo_fsm_inst_free(mm->gmm_rau_req.fsm);

	if (mm->gmm_rau_req.rau_req)
		msgb_free(mm->gmm_rau_req.rau_req);
}
