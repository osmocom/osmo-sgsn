...
static void lu_compl_vlr_new_tmsi(struct osmo_fsm_inst *fi)
{
	...
	osmo_tdef_fsm_inst_state_chg(fi, LU_COMPL_VLR_S_WAIT_TMSI_CNF, lu_compl_fsm_state_tdef, vlr_tdefs, 6);
...
static void vlr_loc_upd_node1_pre(struct osmo_fsm_inst *fi)
{
	...
	if (vlr->cfg.check_imei_rqd && vlr->cfg.retrieve_imeisv_early) {
		osmo_tdef_fsm_inst_state_chg(fi, VLR_ULA_S_WAIT_HLR_CHECK_IMEI_EARLY, lu_fsm_state_tdef, vlr_tdefs, 0);
...
static void vlr_loc_upd_want_imsi(struct osmo_fsm_inst *fi)
{
	...
	/* Obtain_IMSI_VLR */
	osmo_tdef_fsm_inst_state_chg(fi, VLR_ULA_S_WAIT_IMSI, lu_fsm_state_tdef, vlr_tdefs, 0);
			...
			void *data)
{
	...
	if ((!vlr->cfg.retrieve_imeisv_early)
		...
		osmo_tdef_fsm_inst_state_chg(fi, VLR_ULA_S_WAIT_IMEISV, lu_fsm_state_tdef, vlr_tdefs, 0);
	}
}
