...
struct vlr_instance *vlr_alloc(void *ctx, const struct vlr_ops *ops, bool is_ps)
{
	...
	OSMO_ASSERT(is_ps || ops->location_area_served);
	OSMO_ASSERT(is_ps || ops->tx_pvlr_request_lai);

	OSMO_ASSERT(!is_ps || ops->routing_area_served);
	OSMO_ASSERT(!is_ps || ops->tx_pvlr_request_rai);
